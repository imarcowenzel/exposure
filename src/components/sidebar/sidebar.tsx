...
import { sidebarMenuItems, type SidebarMenuItem } from "@/config";
    ...
    <aside className="sticky left-0 top-0 z-20 hidden h-screen w-40 flex-col overflow-hidden bg-black text-white lg:flex">
      ...
      <nav className="mb-24 w-full">
        <div className="flex flex-col items-start gap-y-3">
          ...
          {sidebarMenuItems.default.map((item: SidebarMenuItem) => (
            <div
              key={item.label}
      ...
      {session ? (
        <nav className="mt-auto w-full pb-12">
          <div className="flex flex-col items-start gap-y-3">
            ...
            {sidebarMenuItems.logged.map((item) => (
              <div
                key={item.label}
                className={buttonVariants({
                  variant: "ghost",
        ...
        <nav className="mt-auto px-4 pb-12">
          <div className="flex w-full flex-col gap-y-4">
            ...
            {sidebarMenuItems.notLogged.map((item) => (
              <Link
                key={item.label}
                href={item.href}
                className={
